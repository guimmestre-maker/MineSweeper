...
public class CelulaNr extends Celula {
    ...
    public CelulaNr(int linha, int coluna, int minasVizinhas) {
        ...
        // Uma célula com número só faz sentido num tabuleiro de duas dimensões normais (coordenadas positivas).
        if (linha < 0 || coluna < 0) {
            throw new IllegalArgumentException("As coordenadas da célula numérica não podem ser negativas!");
        }

        // Uma célula de número tem de ter pelo menos 1 mina à volta (senão seria uma CelulaVazia) e no máximo 8 minas (o limite físico de vizinhos numa matriz)
